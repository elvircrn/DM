...
public:
    BipartiteMatchingSolver(const Graph<T> &_g,
    ...
    BipartiteMatchingSolver<T> Draw()
    {
        auto v = g.GetVertices();
        DrawTable<T>(std::vector<T>(v.begin(), v.end()),
                rowNames,
    ...
    BipartiteMatchingSolver<T> DrawFilter()
    {
        auto v = g.GetVertices();
        v.erase(v.find("D"));
        v.erase(v.find("M"));
    ...
    BipartiteMatchingSolver<T> DrawMatch()
    {
        auto v = g.GetVertices();
        v.erase(v.find("D"));
        v.erase(v.find("M"));
...
int main()
{
    ...
    while (scanf("%s", u) == 1)
    {
        ...
        while (std::cin.peek() != '\n')
        {
            ...
            flow[patch::ToString(u)][patch::ToString(v)] = 1;
    ...
    (BipartiteMatchingSolver<std::string>(g, flow, source, sink)).Draw().DrawFilter().DrawMatch();
    std::cout << (BipartiteMatchingSolver<std::string>(g, flow, source, sink)).GetFlow() << '\n';
