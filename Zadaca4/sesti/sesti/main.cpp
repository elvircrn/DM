...
public:
    BellmanFordSolver(const Graph<T> &_g,
                      const T &start,
                      const T &end,
                      bool rev = false) : g(_g)
    {
...
{
    Graph<T> g;
    std::map<T, std::map<T, int>> flow, m;
    T source, sink;
    std::set<T> vertices;
    int iteration;
    std::vector<std::vector<std::string>> mat;
    ...
    int Bfs()
    {
        ...
        while (!q.empty())
        {
            ...
            {
                T next = e.v;
                if (!visited[next] && flow[node][next])
                {
                    visited[next] = true;
                    q.push(next);
                    parent[next] = node;
        ...
        while (parent[node] != node)
        {
            aug = std::min(aug, flow[parent[node]][node]);
            node = parent[node];
        }
        ...
        while (parent[node] != node)
        {
            flow[parent[node]][node] -= aug;
            flow[node][parent[node]] += aug;
            node = parent[node];
        }
        ...
        for (auto& x : vertices)
        {
            if (first)
            {
                rowNames.push_back("Iteracija " + patch::ToString(++iteration));
                mat.push_back(std::vector<std::string>());
                for (auto& y : vertices)
                    mat.back().push_back("");
                first = false;
            ...
            rowNames.push_back(patch::ToString(x));
            first = false;
            mat.push_back(std::vector<std::string>());
            for (auto& y : vertices)
                mat.back().push_back(patch::ToString(flow[x][y]));
        }

        return aug == std::numeric_limits<int>::max() ? 0 : aug;
    }

    int maxFlow;

public:
    FordFulkersonSolver(const Graph<T> &_g,
            const std::map<T, std::map<T, int>> &_flow,
            const T &_source,
            const T &_sink) : g(_g), flow(_flow), source(_source), sink(_sink)
    {
        iteration = 0;
        m = g.GetMatrix();
        vertices = g.GetVertices();
        ...
        //std::cout << "Max flow grafa \\(G\\) je " << maxFlow << '\n';
    }

    int GetFlow() { return maxFlow; }

    FordFulkersonSolver<T> Draw()
    {
        auto v = g.GetVertices();
        DrawTable<T>(std::vector<T>(v.begin(), v.end()),
                rowNames,
    ...
    FordFulkersonSolver<T> DrawFilter()
    {
        auto v = g.GetVertices();
        v.erase(v.find("S"));
...
public:
    BipartiteMatching(const Graph<T> &_g,
            const std::map<T, std::map<T, int>> &_flow,
            const T &_source,
            const T &_sink) : g(_g), flow(_flow), source(_source), sink(_sink)
    {
    ...
    BipartiteMatching<T> DrawFilter()
    {
        auto v = g.GetVertices();
        v.erase(v.find("S"));
        std::vector<std::vector<std::string>> finalMatrix;
...
int main()
{
    ...
    while (scanf("%s %s %d)", u, v, &w) == 3)
    {
        // u i v su praaaavo mali
        for (int i = 1; i < std::strlen(u) - 1; i++)
            u[i - 1] = u[i];
        u[std::strlen(u) - 2]= '\0';
        v[strlen(v) - 1]= '\0';


        flow[patch::ToString(u)][patch::ToString(v)] += w;
        //flow[patch::ToString(v)][patch::ToString(u)] += w;

        g.AddEdgeDir(patch::ToString(u),
                     patch::ToString(v),
                     w);

        indeg[patch::ToString(v)]++;
        outdeg[patch::ToString(u)]++;
    ...
    g.AddEdgeDir("S", "S1", 10000);
    g.AddEdgeDir("S", "S2", 10000);
    g.AddEdgeDir("S", "S3", 10000);
    ...
    std::cout <<     (FordFulkersonSolver<std::string>(g, flow, source, sink)).GetFlow() << '\n';

    (FordFulkersonSolver<std::string>(g, flow, source, sink)).Draw().DrawFilter();
